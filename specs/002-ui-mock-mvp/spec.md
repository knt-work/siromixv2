...
- Q: What is the implementation fidelity requirement for Visily reference designs? → A: Exact 1:1 implementation — For each Next.js page, use the exact layout, text content (Vietnamese), spacing, colors, and design from the corresponding html/ reference folder. No interpretation or modification of the reference design is allowed. All Vietnamese text, button labels, headings, and UI copy must match the reference HTML exactly. This ensures pixel-perfect consistency with approved Visily designs.
